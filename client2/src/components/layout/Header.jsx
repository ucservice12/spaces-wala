  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled || !isHomePage ? 'bg-white shadow-md' : 'bg-transparent py-4'
      }`}
    >
          ...
          <nav className="hidden md:flex items-center space-x-6">
            {menuItems.map((menu) => (
              <div key={menu.label} className="relative group">
                ...
                >
                  <TypographySmall className="flex items-center gap-1 mb-0">
                    {menu.label}
        ...
        <div className="md:hidden bg-white shadow-lg p-4">
          <div className="flex flex-col space-y-4">
              ...
              <div key={menu.label}>
                ...
                >
                  ...
                  <ChevronDown
                    className={`h-4 w-4 transition-transform duration-300 ${
                      activeMobileDropdown === menu.label ? 'rotate-180' : ''
                    }`}
                  />
                </button>
                {activeMobileDropdown === menu.label && (
                  <div className="pl-4 mt-1 space-y-2">
                    {menu.links.map((link) => (
                      <Link key={link.to} to={link.to}>
                        <TypographyP className="block text-gray-600 py-1 mb-0">
                          {link.label}
                        </TypographyP>
                      </Link>
                    ))}
            ...
            <div className="flex flex-col space-y-2 pt-2">
              <Link to="/login" className="text-center py-2 border rounded-md text-gray-700">
                <TypographyP className="mb-0">Login</TypographyP>
              </Link>
              <Link to="/signup" className="text-center bg-blue-600 text-white py-2 rounded-md">
                <TypographyP className="mb-0 text-white">Sign Up</TypographyP>
              </Link>
            </div>
          </div>
        </div>
